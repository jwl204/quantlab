...
def test_constant_prices_give_constant_nav():
    ...
    res = simulate_portfolio(
        prices, weights, [dates[0]], initial_cash=100.0, cost_bps=0.0
    )
...
def test_higher_cost_lowers_terminal_nav():
    ...
    prices = pd.DataFrame(
        {
            "A": 100 * np.cumprod(1 + rng.normal(0, 0.01, 10)),
            "B": 100 * np.cumprod(1 + rng.normal(0, 0.01, 10)),
        },
        index=dates,
    )
    weights = pd.DataFrame({"A": [0.5] * 10, "B": [0.5] * 10}, index=dates)
    low = simulate_portfolio(prices, weights, dates, 100.0, cost_bps=0.0)["nav"].iloc[
        -1
    ]
    high = simulate_portfolio(prices, weights, dates, 100.0, cost_bps=50.0)["nav"].iloc[
        -1
    ]
    assert high < low
